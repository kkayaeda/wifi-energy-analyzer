...
async function fetchDevices(){
  ...
  try {
    ...
    // renderRows fonksiyonunu her zaman çağır
...
async function startScan(){
  ...
  try {
    ...
    const data = await scanRes.json();  // scan sonrası veriyi al
    renderRows(data.devices || [], data.last_updated);
    
    await fetchDevices();  // scan tamamlandıktan sonra cihazları getir
...
const energyChart = new Chart(ctx, {
  ...
  data: {
    ...
    datasets: [{
      label: 'Energy (kWh)',
      data: [],   // Yarım saatlik enerji değerleri
...
// Backend'den enerji verilerini çek ve grafiği güncelle
async function updateEnergyChart() {
  try {
    const response = await fetch(`${API_BASE}/energy_data`);
